import { DEFAULT_FEATURES, FEATURES_ORDER, setFeature } from "../lib/declarations";
...
interface Props
{
  ...
  setFeaturesState: ( features: ( previousState: PolyDictionary ) => PolyDictionary ) => void
}

export default function FeaturesPage( { featuresState, setFeaturesState }: Props ) {
    ...
    useEffect( () => {
        setButtonEnablesAll( () => {
            ...
            return Object.values( featuresState ).filter( ( feature: boolean ) => feature ).length !== featuresStateLength;
        } );
    }, [ featuresState ] );
