import { DEFAULT_FEATURES, DEFAULT_KEYBINDS, DEFAULT_OPTIONS, DEFAULT_SETTINGS, storage } from "./declarations";
...
export async function retrieveFeaturesFromStorage( setter: ( features: Object ) => void )
{
    const localStorageFeatures = JSON.parse( localStorage.getItem("yt-features") as string );
    setter( localStorageFeatures );

    storage.get( ["features"] )
        .then( ( {features} ) => {
            if ( !features ) throw Error("[BYS] :: Features couldnt be loaded from storage, using defaults");
