...
function addListeners( { progressBar, pbBackground, pbForeground, tooltip }: ListenerProps )
{
    const ytShorts = getVideo();
    if ( ytShorts === null ) return;
    ...
    progressBar.addEventListener("mousemove", ( e: MouseEvent ) => {
        ...
        const timestamp = ( (x / ytShorts.clientWidth) * ytShorts.duration ).toFixed(1);
        tooltip.textContent = `${timestamp}s`;

        // Ensure tooltip stays visible at edges of client
        if ((x - (tooltip.offsetWidth / 2)) > (ytShorts.clientWidth - tooltip.offsetWidth)) 
        {
